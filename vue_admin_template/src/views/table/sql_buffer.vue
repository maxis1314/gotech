...
<el-tabs v-model="activeName"  @tab-click="handleClick" ref="table">
      ...
      <el-table-column align="center" label="ID" width="95" className="my-cell" sortable prop="id">
        <template slot-scope="scope">
           ...
           <el-link :underline="false" @click="download_id(scope.row.id)"></el-link>
               ...
               <el-table-column label="Content"   width="" className="my-cell" sortable prop="content">
               <template slot-scope="scope"><span v-html="showHighlight(scope.row.content)"></span>
         <!--
            <no-ssr>
               <mavon-editor
    ...
    >
      ...
      <el-table-column align="left" label="List" className="my-cell">
        <template slot-scope="scope">        
            ...
            <div><!-- <el-tag type="success" size="mini">Content</el-tag><br>-->
            <span v-html="showHighlight(scope.row.content)"></span>
        <!--
         <no-ssr>
               <mavon-editor
                v-model="scope.row.content"
    ...
    <p  style="text-align:center; line-height:10px;">{{ totalCount }}</p>
      ...
      <el-collapse-item v-for="(item,index) in list" :title="'['+item.id +']'"  name="1">
            ...
            <div><el-tag type="success" size="mini">Content</el-tag><br>
            <span v-html="showHighlight(item.content)"></span>
        <!--
         <no-ssr>
               <mavon-editor
    ...
    />
     ...
     <el-tab-pane label="Graph" name="fourth">
      <div id='myChart_sql_buffer' style='width：600px;height:600px'></div>     
    </el-tab-pane>
     ...
     <el-tab-pane label="Form" name="fifth">
        ...
        <el-form :model="form"  label-width="100px">
            ...
            <el-form-item label="Content">
               <el-input v-model="form.content" autocomplete="off" type="" :autosize="{ minRows: 2, maxRows: 3}" @keyup.enter.native="do_submit" />
        <!--
        <div><mavon-editor  ref=md  v-model="form.content" @imgAdd="$imgAdd"  @save="saveDoc" /></div>
        -->
    ...
    $imgAdd(pos, $file){
     ...
     drawLine(){
        axios({
           ...
           console.log(url)
           let myChart = echarts.init(document.getElementById('myChart_sql_buffer'))
