...
<el-tabs v-model="activeName"  @tab-click="handleClick" ref="table">
      ...
      <el-table-column align="center" label="ID" width="95" className="my-cell" sortable prop="id">
        <template slot-scope="scope">
           ...
           <el-link :underline="false" @click="download_id(scope.row.id)"></el-link>
               ...
               <el-table-column label="Line"   width="" className="my-cell" sortable prop="line">
               <template slot-scope="scope"><span v-html="showHighlight(scope.row.line)"></span>
         <!--
            <no-ssr>
               <mavon-editor
         ...
         -->
               ...
               <el-table-column label="Predict"   width="" className="my-cell" sortable prop="predict">
               <template slot-scope="scope"><span v-html="showHighlight(scope.row.predict)"></span>
         <!--
            <no-ssr>
               <mavon-editor
                v-model="scope.row.predict"
                :subfield="false"
                :boxShadow="false"
                defaultOpen="preview"
                :toolbarsFlag="false"
    ...
    >
      ...
      <el-table-column align="left" label="List" className="my-cell">
        <template slot-scope="scope">        
            ...
            <div><!-- <el-tag type="success" size="mini">Line</el-tag><br>-->
            <span v-html="showHighlight(scope.row.line)"></span>
        <!--
         <no-ssr>
               <mavon-editor
                v-model="scope.row.line"
        ...
        -->
            ...
            <div><!-- <el-tag type="success" size="mini">Predict</el-tag><br>-->
            <span v-html="showHighlight(scope.row.predict)"></span>
        <!--
         <no-ssr>
               <mavon-editor
                v-model="scope.row.predict"
    ...
    <p  style="text-align:center; line-height:10px;">{{ totalCount }}</p>
      ...
      <el-collapse-item v-for="(item,index) in list" :title="'['+item.id +']'"  name="1">
            ...
            <div><el-tag type="success" size="mini">Line</el-tag><br>
            <span v-html="showHighlight(item.line)"></span>
        <!--
         <no-ssr>
               <mavon-editor
                v-model="item.line"
        ...
        -->
            ...
            <div><el-tag type="success" size="mini">Predict</el-tag><br>
            <span v-html="showHighlight(item.predict)"></span>
        <!--
         <no-ssr>
               <mavon-editor
                v-model="item.predict"
                :subfield="false"
                :boxShadow="false"
                defaultOpen="preview"
                :toolbarsFlag="false"
    ...
    />
     ...
     <el-tab-pane label="Graph" name="fourth">
      <div id='myChart_intel' style='width：600px;height:600px'></div>     
    </el-tab-pane>
     ...
     <el-tab-pane label="Form" name="fifth">
        ...
        <el-form :model="form"  label-width="100px">
            ...
            <el-form-item label="Line">
               <el-input v-model="form.line" autocomplete="off" type="" :autosize="{ minRows: 2, maxRows: 3}" @keyup.enter.native="do_submit" />
        <!--
        <div><mavon-editor  ref=md  v-model="form.line" @imgAdd="$imgAdd"  @save="saveDoc" /></div>
        -->
        ...
        -->
            ...
            <el-form-item label="Predict">
               <el-input v-model="form.predict" autocomplete="off" type="" :autosize="{ minRows: 2, maxRows: 3}" @keyup.enter.native="do_submit" />
        <!--
        <div><mavon-editor  ref=md  v-model="form.predict" @imgAdd="$imgAdd"  @save="saveDoc" /></div>
        -->
    ...
    $imgAdd(pos, $file){
     ...
     drawLine(){
        axios({
           ...
           console.log(url)
           let myChart = echarts.init(document.getElementById('myChart_intel'))
