...
<el-tabs v-model="activeName"  @tab-click="handleClick" ref="table">
      ...
      <el-table-column align="center" label="ID" width="95" className="my-cell" sortable prop="id">
        <template slot-scope="scope">
           ...
           <el-link :underline="false" @click="download_id(scope.row.id)"></el-link>
               ...
               <el-table-column label="Title"   width="" className="my-cell" sortable prop="title">
               <template slot-scope="scope"><span v-html="showHighlight(scope.row.title)"></span>
         <!--
            <no-ssr>
               <mavon-editor
         ...
         -->
               ...
               <el-table-column label="Content"   width="" className="my-cell">
    ...
    >
      ...
      <el-table-column align="left" label="List" className="my-cell">
        <template slot-scope="scope">        
            ...
            <div><!-- <el-tag type="success" size="mini">Title</el-tag><br>-->
            <span v-html="showHighlight(scope.row.title)"></span>
        <!--
         <no-ssr>
               <mavon-editor
                v-model="scope.row.title"
    ...
    <p  style="text-align:center; line-height:10px;">{{ totalCount }}</p>
      ...
      <el-collapse-item v-for="(item,index) in list" :title="'['+item.id +']'"  name="1">
            ...
            <div><el-tag type="success" size="mini">Title</el-tag><br>
            <span v-html="showHighlight(item.title)"></span>
        <!--
         <no-ssr>
               <mavon-editor
                v-model="item.title"
    ...
    />
     ...
     <el-tab-pane label="Graph" name="fourth">
      <div id='myChart_blog2' style='width：600px;height:600px'></div>     
    </el-tab-pane>
     ...
     <el-tab-pane label="Form" name="fifth">
        ...
        <el-form :model="form"  label-width="100px">
            ...
            <el-form-item label="Title">
               <el-input v-model="form.title" autocomplete="off" type="" :autosize="{ minRows: 2, maxRows: 3}" @keyup.enter.native="do_submit" />
        <!--
        <div><mavon-editor  ref=md  v-model="form.title" @imgAdd="$imgAdd"  @save="saveDoc" /></div>
        -->
    ...
    <el-dialog title="Form" :visible.sync="dialogFormVisible" :close-on-click-modal="false">
      <el-form :model="form"  label-width="100px">
       ...
       <!--
         <el-input v-model="form.content" autocomplete="off" type="" :autosize="{ minRows: 2, maxRows: 3}" />
       -->
       <div><mavon-editor  ref=md  v-model="form.content" @imgAdd="$imgAdd"  @save="saveDoc" /></div>
    ...
    $imgAdd(pos, $file){
     ...
     drawLine(){
        axios({
           ...
           console.log(url)
           let myChart = echarts.init(document.getElementById('myChart_blog2'))
